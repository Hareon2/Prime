...
function adaptive_header(w,h){
    ...
    var headerContancts=$('.header-top-lang');
    if (w<767){
        if (!headerContancts.hasClass('done')){
            headerContancts.addClass('done').appendTo(headerMenu);
        }
    }else{
        if (headerContancts.hasClass('done')){
            headerContancts.removeClass('done').prependTo($('.header-top'));
        }
    }
    if (w<767){
